...
using LevelsCreationExcel.Utils;
using System;
...
{
    public class ColumnCreateViewModel : INotifyPropertyChanged 
    {
        ...
        Autodesk.Revit.DB.Document Doc = OpenColumnWindowCommand.Doc;
        ...
        public HashSet<string> Familiesnames { get; set; } = new HashSet<string>();

        public IList<string> dwgfilesname { get; set; } = new List<string>();
        ...
        private string _selected_dwg;
        public string SelectedDwg
        {
            get { return _selected_dwg; }
            set
            {
        ...
        #region Commands
        public MyCommand CreateColumnCommand { get; set; }
        #endregion

        #region Constructor
        public ColumnCreateViewModel()
        {
            CadLoad();
            CreateColumnCommand = new MyCommand(ExecuteCreateColumnCommand, CanExecuteCreateColumnCommand);
        }
        #endregion
        ...
        public void CadLoad()
        {
            IList<ElementId> cadimports = CadImport.GetImportedCad(Doc);
            FilteredElementCollector collector_of_Dwg = new FilteredElementCollector(Doc).OfClass(typeof(ImportInstance));
            ImportInstance importInstance =null;

            foreach (Element elem in collector_of_Dwg)
            {
                importInstance = elem as ImportInstance;
                if (importInstance != null && importInstance.Category.Name.ToLower().Contains("dwg") && !importInstance.IsLinked) 
                {
                    dwgfilesname.Add(importInstance.Category.Name);
                }
            }
            ...
            Category columnCategory = Category.GetCategory(Doc, BuiltInCategory.OST_StructuralColumns);

            // Get the FamilyManager for the category

            // Get all the loaded families in the project
            IList<Family> loadedFamilies = new FilteredElementCollector(Doc)
                .OfClass(typeof(Family))
                .Cast<Family>()
                .Where(f => f.FamilyCategory.Id == columnCategory.Id)
                .ToList();
            
            foreach(Family family in loadedFamilies)
            {
                Familiesnames.Add(family.Name);
            }

            IList<string> columnTypes = new List<string>();

            FilteredElementCollector collector = new FilteredElementCollector(Doc);
            ICollection<Element> elements = collector.OfCategory(BuiltInCategory.OST_StructuralColumns).ToElements();
        ...
        public void ExecuteCreateColumnCommand(object parameter)
        {
            try
            {
                ...
                foreach (PolyLine pl in plines)
                {
                    ...
                    if (layer == _selectedlayer)
                    {
                        ...
                        if (flag)
                        {
                            FamilySymbol familySymbol = new FilteredElementCollector(Doc).OfCategory(BuiltInCategory.OST_StructuralColumns).WhereElementIsElementType().Where(e => e.Name == SelectedFamily) as FamilySymbol;
                        ...
                        using (Transaction trans = new Transaction(Doc, "create columns"))
                        {
                            ...
                            try
                            {
                                ...
                                FamilyInstance col = Doc.Create.NewFamilyInstance(midline, fs, collevel, StructuralType.Column);

                                // Starting point at the origin
                                XYZ startPoint = midline;
                                // Ending point (positive Z-axis direction)
                                XYZ endPoint = new XYZ(midline.X, midline.Y, Math.Abs(midline.Z * 2));
                                //connect points to line
                                Line zAxisLine = Line.CreateBound(startPoint, endPoint);
                                // Specify the rotation angle (90 degrees)
                                double angle = Math.PI / 2;
                                //rotate the family instance 
                                Location loc = col.Location;
                                loc.Rotate(zAxisLine, angle);
                            }
                            catch (Exception ex)
                            {
                ...
                foreach (Arc ar in arcs)
                {
                    ...
                    if (layer == _selectedlayer)
                    {
                        ...
                        //            fss.Activate();
                        //        }
                        //        Doc.Create.NewFamilyInstance(c, fss, collevel, StructuralType.Column);
                        //    }
                        //    catch (Exception ex)
                        //    {
                        //        TaskDialog.Show(ex.Message, ex.ToString());
        ...
        public bool CanExecuteCreateColumnCommand(object parameter)
        {
            return true;
        }
